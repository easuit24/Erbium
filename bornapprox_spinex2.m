%%% maybe consider both options for spin exchange? Is that already
%%% accounted for???? 
clear all 
tic; 
units 
...
for iBF = 1:length(BFields_gauss_array)
    ...
    beta_i = hbar*ki(1)*avgcrosssection_se/mass*l0^3/tau0;      
    %beta_se(iEn) = hbar*ki(1)*avgcrosssection_se/mass*l0^3/tau0; 
    beta_se(iBF) = hbar*ki(1)*avgcrosssection_se/mass*l0^3/tau0; 
