...
T_matrix = zeros(length(energies), maxLstorage); 
Lmatindex = 1; 
            ...
            for f = 1:numopen
                ...
                if is_elastic_channel
                    ...
                    if Lmatindex <= maxLstorage
                        ...
                        T_matrix(iEn, Lmatindex) = T_matrix_element; 
                    end 
...
sigma_total_elastic_scaled = (sigma_total_elastic/(max(max(C3mat))*mass)^2)/2; 
disp("Time")
disp(toc) 
...
% plot the T-matrix
figure; 
loglog(energies*t0, abs(T_matrix(:,1)).^2*l0^2);
hold on
for ind = 1:size(partial_sigma_arr,2)
