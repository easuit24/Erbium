...
for iBF  = iBFstart :  numBF
    ...
    % --- Diagnostics ---
%     fprintf('B = %.3f G, numopen = %d, is = %d\n', BField*b0, numopen, is);
%     Sii = Smat(is,is);
%     fprintf('  |Sii| = %.6e, arg(Sii) = %.3f rad\n', abs(Sii), angle(Sii));
%     % check flux into open-subspace from this incident column
%     colsum = sum(abs(Smat(:,is)).^2);
%     fprintf('  sum_j |S_{j,i}|^2 = %.6e  (should be <= 1 for loss only to open channels)\n', colsum);
    %%%%%%%
    ...
    for j = 1:numopen
        ...
        if j == is
            ...
            disp(is) 
            crosssection = g*pi*abs(1 - Sji)^2/ki^2;
        else
            % inelastic
            crosssection = g*pi*abs(Sji)^2/ki^2;
        end

        Kmat_channels(iBF,j) = hbar*ki * crosssection/mass;
    end
...
loglog(BFields*b0, Kmat_plot, 'r-', 'LineWidth', 2);
hold on
