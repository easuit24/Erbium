...
rstart = 4.0; % make sure that this is converged!! 
dr = 0.001;
rgo = 100000.0;
%rgo = 100000.0;
...
% for this - need to have resonance at an accessible spin exchange channel 
% but first just run again.... accidentally double squared this. run at -4 -4  
setup_mod
    ...
    for f = 1:numopen
        ...
        if m1_final_target == m1_f && m2_final_target == m2_f 
            ...
            if Lmatindex <= maxLstorage
                T_matrix_BField(iBF, Lmatindex) = ...
                    T_matrix_BField(iBF, Lmatindex) + T_sq;
            end
...
%%
figure; 
loglog(BFields_gauss_array, abs(T_matrix_complete(:,1)), "-g", "LineWidth", 2, "DisplayName","full");
hold on; 
loglog(BFields_gauss_array, abs(T_matrix_BField(:,1)), "-b", "LineWidth", 2, "DisplayName","s-wave");
loglog(BFields_gauss_array, abs(T_matrix_BField(:,2)), "-r", "LineWidth", 2, "DisplayName","p-wave");
legend show
%% 
