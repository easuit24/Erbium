clear all 
load_Er2
folderName = 'Tmatanalysis_fullvsdipolevsBorn'; 
mkdir(folderName);   
...
T_matrix_full = zeros(length(energies), maxLstorage); 
Lmatindex = 1; 
            ...
            for f = 1:numopen
                ...
                if is_elastic_channel
                    ...
                    if Lmatindex <= maxLstorage
                        ...
                        T_matrix_full(iEn, Lmatindex) = T_matrix_element; 
                    end 
...
% plot the T-matrix
figure; 
loglog(energies*t0, abs(T_matrix_full(:,1)).^2*l0^2);
hold on
for ind = 1:size(partial_sigma_arr,2)
    loglog(energies*t0, abs(T_matrix_full(:,ind)).^2*l0^2)
end
