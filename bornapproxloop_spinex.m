...
% m2_incident = -10; 
%L_incident = 4; 
L_incident = 0; % changing this from 0 from spin exchange 
ML_incident = 0;
...
prefactor = reducedmatrix^2*(-sqrt(30)*(muB*gfactor*6)^2*(gfactor*muB)^2); % array for all the different radii away
...
for iEn = 1:length(energies)
    ...
    S_mat(iEn) = sqrt(mass*abs(beta_se)*ki/pi); 
