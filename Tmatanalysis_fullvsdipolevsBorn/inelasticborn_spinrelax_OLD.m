...
for iEn = 1:length(energies)
    ...
    % 1. Identify Incident Channel EXACTLY based on your defined parameters
    is = []; 
    for i = 1:numopen
        ...
        if QN_open(i,1) == m1_incident && ...
           ...
           is = i; 
           break; % Found it, exit this small loop
        end 
    end 
    
    % If the incident channel isn't open yet, skip to the next energy
    if isempty(is)
        continue; 
    end

    % 2. Loop over all Open Outgoing Channels to find RELAXATION
    for f = 1:numopen
        m1_f = QN_open(f,1);
        m2_f = QN_open(f,2);
        ...
        if (m1_f + m2_f) < (m1_incident + m2_incident)
            ...
            % Map to the matrix index (Matches your Born code exactly)
            % Physical L' = 0 -> Col 1, L'=2 -> Col 3, L'=4 -> Col 5, L'=6 -> Col 7
