...
for iEn = 1:length(energies)
    ...
    for lp = 0:4:Lmax 
        ...
        T_sq_exchange_total(iEn) = T_sq_exchange_total(iEn) + 2*T_sq;
...
q = mi-mf; 
angIntegral = sqrt(9*(li+1)*(lf+1)) *thrj(li, 4, lf, 0, 0 ,0)* thrj(li, 4, lf, -mi, q, mf); 
end 
...
radialComponent = numerator/denominator/8; 
end 
