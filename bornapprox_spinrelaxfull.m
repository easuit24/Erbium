...
energies = 3.2e-9; 
%energies = logspace(-9,-7, 5); % shift energy lower for the higher incident channel
...
for iBF = 1:length(BFields_gauss_array)
    BField = BFields_gauss_array(iBF)/b0;  
    thresholds = diag(HBmat)*BField;
    iEn = 1; 
    for im1 = 1:length(m1_final_vals)
        for im2 = 1:length(m2_final_vals)
            if m1_final_vals(im1) == -8 && m2_final_vals(im2) == -8
                continue 
            end 
            % scattering result 

            m1_final = m1_final_vals(im1); 
            m2_final = m2_final_vals(im2); 
            ...
            for l = 0:2:Lmax
                for lp = 0:2:Lmax 
                    ...
                    Ra = rad_integral(l, lp, ki(1), kf); % these are magnitudes of k
                    % fprintf('Sigma Bar: %f\n', sigma_bar)
                    % disp(C) 
                    % disp(Ra) 
            ...
            avgcrosssection_sr = abs(avgcrosssection_sr) + (mass^2/(2*pi)^2)*abs(sigma_bar)*angular_component; 
            disp(avgcrosssection_sr)
             
        end
    end 
    beta_i = hbar*ki(1)*avgcrosssection_sr/mass*l0^3/tau0;
    beta_sr(iBF) = hbar*ki(1)*avgcrosssection_sr/mass*l0^3/tau0; 
    %beta_sr(iEn) = hbar*ki(1)*avgcrosssection_sr/mass*l0^3/tau0; 
    fprintf('Angular Cross Section (Spin Relaxation): %f\n', avgcrosssection_sr)
    fprintf('Elapsed Time: %f\n', toc)
    
    S_mat(iEn) = sqrt(mass*abs(beta_i)*ki(1)/pi); 
end
