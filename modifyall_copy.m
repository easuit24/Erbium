clear all 
load_Er2
folderName = 'curveanalysis4'; % Define the desired folder name
mkdir(folderName);   
...
tic; 
MeanDe_arr = linspace(100,110,2); % just have a fixed depth for now
...
% Should have 49 variables to loop over 
for Ombar = 0 : 2 : j1 + j2 % loop over projection omega  
    for j12 = Ombar : 4 : j1 + j2 % loop over total angular momenta
        filename = sprintf('curvedata_%.2f_%.2f.mat', Ombar, j12);
        raterationame = sprintf('raterat_%.2f_%.2f.mat', Ombar, j12); 
        variableFullPath = fullfile(folderName, filename);
        variableFullPath2 = fullfile(folderName, raterationame); 
        ...
        for md = 1:length(MeanDe_arr)    
            ...
            rng(9)
            Index = 0;
            for Ombar_i = 0 : 2 : j1 + j2
                for j12_i = Ombar_i : 4 : j1+j2
                    Index = Index + 1;
                    De = MeanDe + DeltaDe*(rand-0.5);
                    C12 = C6^2/4/De;
                    % short range coefficient matrix - for short range interactions 
                    SRcoef(Ombar_i+1,j12_i+1) = C12-MeanC12;   %  +1 since Ombar, j12 can be zero
                end
            end
            ...
            %Ombarchange = (j1+j2-curve_index);  
            %SRcoef(Ombarchange+1,Ombarchange+1)
            De = MeanDe + DeltaDe*(rand-0.5);
            C12 = C6^2/4/De;
            %SRcoef(Ombarchange+1,Ombarchange+1) = C12-MeanC12;
            SRcoef(Ombar+1, j12+1) = C12-MeanC12; % change the specific index 
        ...
        save(variableFullPath2 , "mean_tot", "unc_tot")
        save(variableFullPath, "BFields_gauss_array", "spin_sr_arr", "spin_ex_arr")
    end 
end 
